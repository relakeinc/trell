    ...
    return (
      <div className={`fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4 ${overlayCls}`}>
        <div className={`trell-card w-full max-w-lg p-6 ${cardCls}`}>
          ...
          <label className="mt-4 block text-xs font-medium text-trell-muted">
            Publishable key (pk) — safe for the SDK
          </label>
          <CopyRow value={keys.pk} />
          <label className="mt-3 block text-xs font-medium text-trell-muted">
...
function CopyRow({ value, mono }: { value: string; mono?: boolean }) {
  return (
    <div className="mt-1 flex items-center gap-2">
      <code
        className={`flex-1 truncate rounded-md border border-trell-line bg-trell-bg px-3 py-2 text-xs ${mono ? "font-mono" : ""}`}
        title={value}
      >
        {value}
      </code>
      <button onClick={() => void navigator.clipboard.writeText(value)} className="trell-btn-outline h-8 px-2 text-xs">
        Copy
      </button>
    </div>
  );
