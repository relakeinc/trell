...
export function ProjectSidebar({
  ...
  function prefetchPage(href: string) {
    ...
    if (href === "analytics") {
      queryClient.prefetchQuery({
        queryKey: ["stats", pid, qs],
        queryFn: () => fetch(`/api/projects/${pid}/stats?${qs}`).then((r) => r.json()),
      });
      queryClient.prefetchQuery({
        queryKey: ["series", pid, "day", qs],
        queryFn: () => fetch(`/api/projects/${pid}/series?interval=day&${qs}`).then((r) => r.json()),
      });
      queryClient.prefetchQuery({
        queryKey: ["breakdown", pid, "page", qs],
        queryFn: () => fetch(`/api/projects/${pid}/breakdown?dimension=page&${qs}`).then((r) => r.json()),
      });
      queryClient.prefetchQuery({
        queryKey: ["forms", pid, qs],
        queryFn: () => fetch(`/api/projects/${pid}/forms?${qs}`).then((r) => r.json()),
      });
      queryClient.prefetchQuery({
        queryKey: ["events", pid, qs, 15],
        queryFn: () => fetch(`/api/projects/${pid}/events?limit=15&${qs}`).then((r) => r.json()),
      });
    } else if (href === "events") {
      queryClient.prefetchQuery({
        queryKey: ["events", pid, qs, 50],
        queryFn: () => fetch(`/api/projects/${pid}/events?limit=50&${qs}`).then((r) => r.json()),
      });
    } else if (href === "submissions") {
      queryClient.prefetchQuery({
        queryKey: ["submissions", pid],
        queryFn: () => fetch(`/api/projects/${pid}/submissions`).then((r) => r.json()),
      });
    } else if (href === "funnels") {
      queryClient.prefetchQuery({
        queryKey: ["funnels", pid],
        queryFn: () => fetch(`/api/projects/${pid}/funnels`).then((r) => r.json()),
      });
    }
  }
  ...
  return (
    <>
          ...
          <div className="flex flex-col p-3">
            <div className="mb-5 px-1" ref={dropdownRef}>
              ...
              >
                ...
                <span className="flex items-center gap-1.5">
                  ...
                  <span
                    className={`flex size-5 shrink-0 items-center justify-center rounded-md bg-neutral-200 transition-all duration-200 ${dropdownOpen ? "rotate-180" : ""}`}
                  >
            ...
            {NAV_SECTIONS.map((section, si) => (
              <div key={si} className="mb-6 flex flex-col gap-1">
                {section.label && <div className="mb-2 px-3 text-[13px] text-neutral-400">{section.label}</div>}
            ...
            <div className="mb-6 flex flex-col gap-1">
              <div className="mb-2 px-3 text-[13px] text-neutral-400">Settings</div>
              <Link
        ...
        <div className="flex flex-shrink-0 flex-col gap-2.5 border-t border-neutral-200 px-4 py-3">
          ...
          <div className="flex items-center justify-between pt-0.5">
            <div className="flex items-center gap-2">
              ...
              <span className="max-w-[130px] truncate text-[13px] text-neutral-400">{userEmail}</span>
            </div>
      ...
      <CreateProjectModal open={createOpen} onClose={() => setCreateOpen(false)} onCreated={() => router.refresh()} />
    </>
  );
}
