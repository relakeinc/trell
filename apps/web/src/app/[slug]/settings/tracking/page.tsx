  ...
  return (
    <div className="flex flex-col gap-6">
          ...
          <>
            <div className="p-4">
              ...
              <p className="mb-3 text-sm text-trell-ink-muted">
                Add this snippet to the <code className="rounded bg-neutral-100 px-1 py-0.5 text-xs">&lt;head&gt;</code>{" "}
                of your HTML to start tracking events automatically.
              </p>
            ...
            <div className="flex items-center justify-between border-t border-trell-line bg-neutral-50 px-4 py-2.5">
              <span className="text-xs text-trell-ink-muted">
                {isPkReady ? (
                  ...
                  <>
                    Replace <code className="rounded bg-neutral-100 px-1 py-0.5">pk_YOUR_KEY</code> with your project
                    key.
                  </>
                )}
              </span>
              <button
                ...
                className="trell-btn-outline flex h-8 cursor-pointer items-center gap-1.5 text-xs"
              >
          ...
          <>
            <div className="flex flex-col gap-4 p-4">
              <p className="text-sm text-trell-ink-muted">
                Send events from your backend with a secret key from{" "}
                <Link href={`/${slug}/settings/api`} className="font-medium text-blue-600 hover:underline">
                  API Keys
                </Link>
                . Server keys bypass the domain allowlist — keep them in your{" "}
              ...
              <div>
                <div className="mb-1.5 flex items-center justify-between">
                  ...
                  <button
                    onClick={() => void copySnippet("env", "TRELL_SECRET_KEY=sk_paste_yours")}
                    className="text-xs text-trell-ink-muted transition-colors hover:text-trell-ink"
                  >
                    {copiedSnippet === "env" ? "Copied!" : "Copy"}
                  </button>
                </div>
                <pre className="overflow-x-auto rounded-lg border border-trell-line bg-neutral-950 p-3 font-mono text-xs leading-relaxed text-neutral-300">
                  <code>TRELL_SECRET_KEY=sk_paste_yours</code>
                </pre>
              </div>
              <div>
                <div className="mb-1.5 flex items-center justify-between">
                  <span className="text-xs font-medium text-trell-ink">
                    2. Forward events from your backend (Next.js example)
                  </span>
                  <button
                    onClick={() => void copySnippet("next", nextSnippet)}
                    className="text-xs text-trell-ink-muted transition-colors hover:text-trell-ink"
                  >
              ...
              <div>
                ...
                <pre className="max-h-64 overflow-auto rounded-lg border border-trell-line bg-neutral-950 p-3 font-mono text-xs leading-relaxed text-neutral-300">
            ...
            <div className="flex items-center justify-between border-t border-trell-line bg-neutral-50 px-4 py-2.5">
              <span className="text-xs text-trell-ink-muted">
                Secret keys are created per environment in{" "}
                <Link href={`/${slug}/settings/api`} className="font-medium text-blue-600 hover:underline">
                  API Keys
                </Link>
                .
      ...
      <div className="overflow-hidden rounded-lg border border-trell-line bg-white">
        <div className="flex items-center justify-between px-4 py-3">
          <div>
            ...
            <p className="mt-0.5 text-sm text-trell-ink-muted">
              Automatically track form views, submissions, and page views.
            </p>
          </div>
      ...
      <div className="overflow-hidden rounded-lg border border-trell-line bg-white">
        ...
        <div className="flex flex-col gap-4 p-4">
          <p className="text-sm text-trell-ink-muted">
            Every <code className="rounded bg-neutral-100 px-1 py-0.5 text-xs">&lt;form&gt;</code> is tracked
            automatically and its values are captured on submit.
          </p>
          <div>
            <div className="mb-1.5 text-xs font-medium text-trell-ink">Give your form a stable name (recommended)</div>
            <pre className="overflow-x-auto rounded-lg border border-trell-line bg-neutral-950 p-3 font-mono text-xs leading-relaxed text-neutral-300">
              <code>{`<form data-trell-form-id="contact-form" data-trell-form-name="Contact">`}</code>
            </pre>
            <p className="mt-1.5 text-xs text-trell-ink-muted">
              Without an id, Trell derives a stable id from the form&apos;s structure, but naming it makes your
              analytics readable.
            </p>
          </div>
          <div>
            ...
            <p className="text-xs text-trell-ink-muted">
              On submit, all <code className="rounded bg-neutral-100 px-1 py-0.5">input</code>,{" "}
              <code className="rounded bg-neutral-100 px-1 py-0.5">textarea</code>, and{" "}
              <code className="rounded bg-neutral-100 px-1 py-0.5">select</code> values are sent in{" "}
              <code className="rounded bg-neutral-100 px-1 py-0.5">properties.fields</code> and shown on the Submissions
              page. Password fields are masked as <code className="rounded bg-neutral-100 px-1 py-0.5">***</code>.
            </p>
          </div>
        </div>
      </div>
