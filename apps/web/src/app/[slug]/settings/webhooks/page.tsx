          ...
          >
              ...
              <div className="flex flex-col gap-4">
                <div>
                  ...
                  <input
                    ...
                    className="trell-input w-full max-w-lg"
                  />
                </div>
                <div>
            ...
            <div className="flex items-center justify-between border-t border-trell-line bg-neutral-50 px-4 py-2.5">
              <span className="text-xs text-trell-ink-muted">
                You&apos;ll receive a POST request for each matching event.
              </span>
              <button
                type="submit"
            ...
            {webhooks.length === 0 ? (
              ...
              <div className="flex flex-col gap-2">
                {webhooks.map((w) => (
                  <div
                    key={w.id}
                    className="flex flex-wrap items-center justify-between gap-2 rounded-md border border-trell-line px-3 py-2"
                  >
                    <div className="flex min-w-0 items-center gap-3">
                      ...
                      <div className="min-w-0">
                        ...
                        <div className="text-xs text-trell-ink-muted">
                          {w.events.join(", ")} · {w.enabled ? "Active" : "Paused"}
                        </div>
                      </div>
                    </div>
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      {showSecret === w.id && (
                        <code className="rounded bg-neutral-100 px-2 py-1 text-xs font-mono text-trell-ink-muted">
                          {w.secret}
                        </code>
                      )}
                      <button
          ...
          <div className="p-4">
            {loadingDeliveries ? (
              ...
              <p className="text-sm text-trell-ink-muted">
                No deliveries yet. Webhooks will appear here after events fire.
              </p>
            ) : (
                ...
                {deliveries.map((d) => (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <span
                        className={`inline-block size-2 rounded-full ${d.status === "success" ? "bg-green-500" : d.status === "failed" ? "bg-red-500" : "bg-yellow-400"}`}
                      />
                      <div>
                        <div className="text-sm font-medium text-trell-ink">{d.event}</div>
                        <div className="text-xs text-trell-ink-muted">
                          {d.statusCode ? `HTTP ${d.statusCode}` : d.status} · {d.attempts} attempt
                          {d.attempts !== 1 ? "s" : ""} · {new Date(d.createdAt).toLocaleString()}
                        </div>
                      </div>
                    </div>
                    {d.response && (
                      <code className="max-w-xs truncate text-xs text-trell-ink-muted" title={d.response}>
                        {d.response.slice(0, 80)}
                      </code>
                    )}
                  </div>
                ))}
