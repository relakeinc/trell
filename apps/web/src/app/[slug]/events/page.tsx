...
import { useProjectId, useProjectEvents, type DrillEvent } from "@/lib/hooks";
...
type SortKey = "az" | "events" | "newest" | "oldest";
...
export default function EventsPage() {
  ...
  const { data, isLoading } = useProjectEvents(projectId, qs, 100);
  const events = data?.events ?? [];

  const groups = useMemo<EventGroup[]>(() => {
    const map = new Map<string, EventGroup>();
    for (const e of events) {
      const key = e.formId ? `form:${e.formId}` : `page:${e.pagePath}`;
      const g = map.get(key);
      if (!g) {
        map.set(key, {
          key,
          name: displayName(e) ?? "(No form)",
          formId: e.formId,
          count: 1,
          lastTs: e.ts,
          firstTs: e.ts,
          pages: e.pagePath ? [e.pagePath] : [],
          recent: [e],
        });
      } else {
        g.count += 1;
        ...
        if (e.pagePath && !g.pages.includes(e.pagePath)) g.pages.push(e.pagePath);
        g.recent.push(e);
      }
    }
      ...
      <div className="trell-card overflow-hidden">
        <table className="trell-table trell-table-plain w-full border-separate border-spacing-0 text-sm">
          <thead>
            <tr className="border-b border-trell-line text-left text-[13px] text-neutral-500">
              ...
              <th className="hidden bg-white px-2 pb-2 pt-3 font-normal sm:table-cell">Events</th>
        ...
        {!isLoading && events.length === 0 && (
          <div className="trell-empty-state">
            ...
            <p className="text-sm font-medium text-trell-ink">
