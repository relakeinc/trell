...
export default function NotFound() {
  ...
  return (
    <>
      {/* Esta página no hace scroll: liberamos el gutter que reserva
          `scrollbar-gutter: stable` del layout global para que el arte
          llegue hasta el borde derecho del viewport. */}
      <style>{`html{scrollbar-gutter:auto}`}</style>
      {/* Estilo del botón copiado tal cual de relake-site (allí
          `trell-btn-primary` es azul; en esta app el global es negro).
          Se escopa bajo `.trell-404` para no afectar al resto de la app. */}
      ...
      <div className="flex shrink-0 flex-col items-center px-5 pt-20 text-center sm:pt-28">
        <p className="text-sm font-medium tracking-wide text-[#71717a] dark:text-[#a1a1aa]">
          OOPS! 404 Error!
        </p>
        ...
        <button
          type="button"
          onClick={goBack}
          className="trell-btn-primary mt-6"
        >
          Go Back
        </button>
      </div>
