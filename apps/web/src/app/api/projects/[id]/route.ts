...
export async function PATCH(req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  ...
  if (body.name !== undefined) {
    ...
    if (trimmed.length > 32)
      return NextResponse.json(
        { error: "invalid_name", message: "Name must be 32 characters or less" },
        { status: 400 },
      );
  ...
  if (body.slug !== undefined) {
    ...
    if (!/^[a-z0-9-]+$/.test(trimmed))
      return NextResponse.json(
        { error: "invalid_slug", message: "Only lowercase letters, numbers, and dashes" },
        { status: 400 },
      );
    if (trimmed.length > 48)
      return NextResponse.json(
        { error: "invalid_slug", message: "Slug must be 48 characters or less" },
        { status: 400 },
      );
    if (trimmed !== project.slug) {
      const exists = await prisma.project.findUnique({ where: { slug: trimmed } });
      if (exists)
        return NextResponse.json({ error: "invalid_slug", message: "This slug is already taken" }, { status: 400 });
    }
  ...
  try {
      ...
      if (domain && !list.some((d) => d === domain)) {
        if (list.length >= limits.domains) {
          return NextResponse.json(
            {
              error: "limit_reached",
              message: `Free plan allows up to ${limits.domains} domains. Upgrade to Pro for more.`,
            },
            { status: 403 },
          );
        }
    ...
    return NextResponse.json(
      { error: "invalid_domain", message: e instanceof Error ? e.message : "invalid_domain" },
      { status: 400 },
    );
  }
