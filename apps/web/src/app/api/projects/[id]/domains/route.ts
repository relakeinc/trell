...
export async function POST(
  ...
  if (!domain) return NextResponse.json({ error: "domain is required", message: "Domain is required" }, { status: 400 });
  ...
  try {
    ...
    return NextResponse.json({ error: "invalid_domain", message: "Enter a valid hostname (e.g. example.com)" }, { status: 400 });
  }
  ...
  if (existing.includes(normalized!)) {
    return NextResponse.json({ error: "domain already exists", message: "That domain is already added" }, { status: 409 });
  }

  const { limits } = await getProjectOwnerPlan(id);
  if (existing.length >= limits.domains) {
    return NextResponse.json({ error: "limit_reached", message: `Domain limit reached. Upgrade to Pro for more.` }, { status: 403 });
  }
